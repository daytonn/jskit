...
JSKit is all about making testing javascript easier. JSKit itself comes with some handy conventions for testing JSKit applications. When you create a controller with the `Application.createController` method. The created Controller's constructor will be available on the `Application` object using the Controller's `className`. Using the "Posts" controller example:

```js
App.createController("Posts", { ... });
App.Controllers.PostsController; // Post Controller instance (for production)
App.PostsController; // Post Controller constructor (for testing)
```
...
A dispatcher is required when a controller is constructed, normally `createController` handles this for you but in your tests you'll need to explicitly pass one to the constructor. You should use the `JSKit.Dispatcher` when testing Controllers. To create a controller with the `JSKit.Dispatcher` simply pass it to the Controller:
